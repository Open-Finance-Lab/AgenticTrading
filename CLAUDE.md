...
- `BREVO_API_KEY` / `ACCOUNT_EMAIL_FROM` (both required together) and optional `ACCOUNT_EMAIL_FROM_NAME` (defaults to `Agentic Trading Lab`): transactional email credentials for the two-code account email-change flow (`infrastructure/email/sender.py`). Leave either required key unset and `POST /api/auth/email-change` (both stages) and its verify route return **503**; display-name editing and logout are unaffected. Set both in the **Render dashboard** for prod — `render.yaml` is documentation, not the deploy mechanism (see the Prod deploy reality gotcha below).
