...
The old console (`/app?view=admin`) keeps **Users** (account management and the grant pool), **Providers** and **Activity** (`admin-credits.js`, `admin-model-providers.js`, and `app.js`'s `#adminStats` strip). *(Amended 2026-09-17: Providers ported to `/admin` in the navigation consolidation; `?adminTab=providers` now redirects to `/admin#providers`.)*
